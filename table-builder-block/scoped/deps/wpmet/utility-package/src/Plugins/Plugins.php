...
namespace TableBuilderScopedDeps\Wpmet\UtilityPackage\Plugins;
...
class Plugins
{
    ...
    public function wpmet_apps_renderer()
    {
        ...
        foreach ($this->plugins as $key => $plugin) {
            ...
            $plugin_status_title = isset($plugin_data['title']) ? $plugin_data['title'] : esc_html__('Activate', 'table-builder-block');
            ?>
            ...
            if (!empty($plugin_docs)) {
                echo sprintf('<a target="_blank" rel="noopener noreferrer" href="%1$s" class="wpmet-onboard-tut-term--help">%2$s</a>', esc_url($plugin_docs), esc_html__('Read Docs', 'table-builder-block'));
            }
            ?>
								</div>
							</label>
