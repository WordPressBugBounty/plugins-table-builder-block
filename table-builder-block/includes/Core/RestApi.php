<?php
/**
 * REST API fallback for plugin dependency management
 *
 * @package TableKit
 */
...
class RestApi {
	...
	/**
	 * Hooks route registration and plugin-cache invalidation into WordPress.
	 */
	...
	/**
	 * Clears the cached plugin-status transient when plugins are (de)activated.
	 *
	 * @return void
	 */
	...
	/**
	 * Registers the tablekit/v1 fallback REST routes, only if the
	 * tablekit-essential plugin (which normally provides them) isn't active.
	 *
	 * @return void
	 */
	public function register_routes() {
		...
		register_rest_route(
			'tablekit/v1',
			'/table-manager-api/check-plugins',
			array(
				'methods'             => 'POST',
		...
		register_rest_route(
			'tablekit/v1',
			'/table-manager-api/activate-plugins',
			array(
				'methods'             => 'POST',
		...
		register_rest_route(
			'tablekit/v1',
			'/table-manager-api/install-plugins',
			array(
				'methods'             => 'POST',
		...
		register_rest_route(
			'tablekit/v1',
			'/table-manager-api/check-pro-license',
			array(
				'methods'             => 'POST',
	...
	/**
	 * Checks whether the tablekit-essential plugin is active.
	 *
	 * @return bool True if tablekit-essential is active.
	 */
	...
	/**
	 * REST callback: report install/active status for a list of plugin slugs.
	 *
	 * @param \WP_REST_Request $request Request with a JSON "plugins" array param.
	 * @return \WP_REST_Response Status per slug: installed/active/state/plugin_file.
	 */
	public function check_plugins_status( $request ) {
		...
		foreach ( $plugins as $slug ) {
			...
			foreach ( $all_plugins as $plugin_path => $plugin_data ) {
				if ( 0 === strpos( $plugin_path, $slug . '/' ) || $slug . '.php' === $plugin_path ) {
	...
	/**
	 * REST callback: activate a list of already-installed plugins by slug.
	 *
	 * @param \WP_REST_Request $request Request with a JSON "plugins" array param.
	 * @return \WP_REST_Response Activation results: which slugs succeeded/failed.
	 */
	...
	/**
	 * REST callback: install (and activate) a list of plugins by slug from
	 * the WordPress.org plugin directory. Slugs containing "-pro" are
	 * rejected, since pro plugins must be purchased/uploaded manually.
	 *
	 * @param \WP_REST_Request $request Request with a JSON "plugins" array param.
	 * @return \WP_REST_Response Installation results: which slugs failed and why.
	 */
	public function install_plugins( $request ) {
		...
		foreach ( $plugins as $slug ) {
			...
			if ( strpos( $slug, '-pro' ) !== false ) {
				$failed[] = array(
					'slug'  => $slug,
					'error' => 'Pro plugins must be purchased manually',
				);
				continue;
			}
	...
	/**
	 * REST callback: check whether GutenKit Pro's license is active
	 * (license checks are currently only supported for that plugin).
	 *
	 * @param \WP_REST_Request $request Request with a JSON "plugins" array param.
	 * @return \WP_REST_Response License status.
	 */
	...
	/**
	 * Builds the data localized to the editor as `tablekitEssential`, mirroring
	 * the shape the tablekit-essential plugin provides, so the editor JS
	 * doesn't need to branch on which one is active. Cached for 12 hours.
	 *
	 * @return array{apiUrl:string,restNonce:string,ajaxNonce:string,activePlugins:string[],installedPlugins:string[],requiredPlugins:string[]}
	 */
